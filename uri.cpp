...
int V[MAXN];
int frequencia[2*ADD+1][2];

vector<int> positivo1,positivo2,p1,p2,negativo1,negativo2;

inline int checa(int val,int id){
    val += ADD;
    if(val < 0) return 0;
    return (frequencia[val][id] == iteracao);
}

void adiciona1(int pos,int val,int bitmask){
    if(pos == N){
        frequencia[val+ADD][0] = iteracao;
        return;
    }
